...
def insert_nested_tables_batch(db_manager, table_name: str, records: List[Dict[str, Any]]):
    """Insert batch records vào các bảng nested với xử lý duplicate keys"""
    for obj in records:
        ...
        for nested_field, nested_list in nested.items():
            ...
            for item in nested_list:
                ...
                # Hàm helper để escape SQL values
                def sql_value(val):
                    if val is None:
                        return 'NULL'
                    if isinstance(val, str):
                ...
                # Kiểm tra xem record đã tồn tại chưa (dựa trên id và fk_id)
                if 'id' in filtered_item:
                    check_sql = f"SELECT COUNT(*) FROM {escape_column_name(sub_table)} WHERE [id] = {sql_value(filtered_item['id'])} AND [fk_id] = {sql_value(filtered_item['fk_id'])}"
                    db_manager.cursor.execute(check_sql)
                    exists = db_manager.cursor.fetchone()[0] > 0
                    
                    if exists:
                        # Update nếu đã tồn tại cùng id và fk_id
                        set_clause = ', '.join([f'{escape_column_name(col)} = {sql_value(filtered_item[col])}' for col in filtered_item.keys() if col != 'id'])
                        update_sql = f"UPDATE {escape_column_name(sub_table)} SET {set_clause} WHERE [id] = {sql_value(filtered_item['id'])} AND [fk_id] = {sql_value(filtered_item['fk_id'])}"
                        try:
                            db_manager.cursor.execute(update_sql)
                        except Exception as e:
                            print(f"  Error khi update nested record: {e}")
                    else:
                        # Tạo UUID mới nếu trùng id nhưng khác fk_id
                        new_id = str(uuid.uuid4())
                        filtered_item['id'] = new_id
                        
                        # Insert với id mới
                        columns = ', '.join([escape_column_name(col) for col in filtered_item.keys()])
                        values = ', '.join([sql_value(filtered_item[col]) for col in filtered_item.keys()])
                        
                        insert_sql = f'INSERT INTO {escape_column_name(sub_table)} ({columns}) VALUES ({values})'
                        try:
                            db_manager.cursor.execute(insert_sql)
                        except Exception as e:
                            print(f"  Error khi insert nested record với UUID: {e}")
                else:
...
def insert_or_update_customer_final(data: Dict[str, Any], table_name: str = "crm_data_customer"):
    """
    Phiên bản cuối cùng xử lý cả bảng chính và các bảng nested
    - Xử lý bảng chính với dữ liệu cơ bản
    - Tạo các bảng nested với fk_id để liên kết
    - Xử lý duplicate keys trong nested tables
    - Tối ưu cho hiệu suất với batch processing
    """
    ...
    try:
        if db_manager.connect():
            if 'data' in data and isinstance(data['data'], list):
                ...
                print(f"Bắt đầu xử lý {total_records} records (bảng chính + nested)...")
                
                # Xử lý theo batch
                batch_size = 50  # Giảm batch size vì có nested tables
